...
async fn publish_to_non_existing_exchange() {
    Steps
        ...
        .when("publish a non-existing exchange", step!(|w: World| {
            w.result = w.conn.basic_publish(1, "non existent", "any key", "data".to_string()).await;
            Ok(())
        }))
...
async fn publish_to_default_exchange() {
    Steps
        ::feature("Publish to default exchange", init!(World, {
            Ok(World {
                conn: client::connect("127.0.0.1:5672").await?,
                result: Ok(())
            })
        ...
        .when("publish a message with empty channel", step!(|w: World| {
            w.result = w.conn.basic_publish(1, "", "any key", "data".to_string()).await;
            Ok(())
        }))
        .then("it goes to the default exchange", step!(|w: World| {
...
async fn publish_to_intenal_exchange() {
    Steps
        ::feature("Publish to internal exchange", init!(World, {
            Ok(World {
                conn: client::connect("127.0.0.1:5672").await?,
                result: Ok(())
            })
        ...
        .when("publish to an internal exchange", step!(|w: World| {
        ...
        .then("it gives a 403 channel error", step!(|w: World| {
            let mut res = Ok(());
            std::mem::swap(&mut w.result, &mut res);
            assert!(res.is_err());

            let err = ironmq_test::to_client_error(res);
