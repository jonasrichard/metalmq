...
impl TestCase {
    ...
    pub async fn new_client(&self) -> TestClient {
        let ctx = Context {
    ...
    pub async fn new_client_with_channel(&self, channel: u16) -> TestClient {
    ...
    pub async fn new_client_with_channels(&self, channels: &[u16]) -> TestClient {
        let mut client = self.new_client().await;
